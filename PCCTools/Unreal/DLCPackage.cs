...
namespace PCCTools.Unreal
{
    public class DLCPackage
    {
        ...
        public void ReBuild()
        {
            ...
            //Console.WriteLine("Creating Header Dummy...");
            for (int i = 0; i < 8; i++)
                fs.Write(BitConverter.GetBytes(0), 0, 4);
            Header.EntryOffset = 0x20;
            //Console.WriteLine("Creating File Table...");
            for (int i = 0; i < Header.FileCount; i++)
            {
            ...
            //Console.WriteLine("Creating Block Table...");
            for (int i = 0; i < Header.FileCount; i++)
            ...
            for (int i = 0; i < Header.FileCount; i++)
            {
                ...
                FileEntryStruct e = Files[i];
                //Console.WriteLine("Rebuilding \"" + e.FileName + "\" (" + (i + 1) + "/" + Header.FileCount + ") " + BytesToString(e.UncompressedSize) + " ...");
                e.DataOffset = pos;
                e.DataOffsetAdder = 0;
            ...
            //Console.WriteLine("Updating FileTable...");
            fs.Seek(0x20, 0);
            ...
            fs.Seek(0, 0);
            //Console.WriteLine("Rebuilding Header...");
        ...
        //    {
        //        FileStream fs = new FileStream(FileName, FileMode.Open, FileAccess.Read);
        //        Console.WriteLine("Searching TOC...");
        //        int f = FindTOC();
        //        if (f == -1)
        //            return;
        //        Console.WriteLine("Found TOC, deleting line...");
        ...
        //        Console.WriteLine("Replacing TOC...");
        //        ReplaceEntry(Encoding.ASCII.GetBytes(toc), f);
        //        Console.WriteLine("Deleting Entry from Filelist...");
        ...
        //        if (f == -1)
        //            return;
        //        Console.WriteLine("Found TOC, deleting lines...");
        //        MemoryStream m = DecompressEntry(f, fs);
        //        string toc = Encoding.UTF8.GetString(m.ToArray(), 0, (int)m.Length);
        //        fs.Close();
        ...
        //    if (f == -1)
        //        return;
        //    Console.WriteLine("Found TOC, adding line...");
        //    MemoryStream m = DecompressEntry(f, fs);
        //    //
        //    //Update TOC
        //    WriteString(m, path);
        ...
        //    long offset = oldsize;
        //    Console.WriteLine("File End Offset : 0x" + offset.ToString("X10"));
        //    fs.Seek(oldsize, 0);
        ...
        //    }
        //    offset += count * 0x1E;
        //    Console.WriteLine("Table End Offset : 0x" + offset.ToString("X10"));
        ...
        //    }
        //    offset = fs.Length;
        //    Console.WriteLine("Block Table End Offset : 0x" + offset.ToString("X10"));
        //    long dataoffset = offset;
        //    fs.Write(FileIN, 0, FileIN.Length);
        //    offset += FileIN.Length;
        //    Console.WriteLine("New Data End Offset : 0x" + offset.ToString("X10"));
        //    //
        //    //Append TOC
        //    long tocoffset = offset;
        //    fs.Write(m.ToArray(), 0, (int)m.Length);
        //    offset = fs.Length;
        //    Console.WriteLine("New TOC Data End Offset : 0x" + offset.ToString("X10"));
        //    //update filetable
        //    fs.Seek(oldsize, 0);
        //    uint blocksizeindex = 0;
        //    for (int i = 0; i < count; i++)
        ...
        public List<TOCBinFile.Entry> UpdateTOCbin(bool Rebuild = false)
        {
            ...
            if (f == -1)
            {
                //Console.WriteLine("Couldnt Find PCConsoleTOC.bin");
                return null;
            }
            int IndexTOC = f;
            //Console.WriteLine("Found PCConsoleTOC.bin(" + f + ")!\nLoading Entries...");
            TOCBinFile TOC = new TOCBinFile(new MemoryStream(DecompressEntry(f).ToArray()));
            //Console.WriteLine("Checking Entries...");
            int count = 0;
            //if (TOC.Entries == null)
            //    Console.WriteLine("No TOC entries found. Oh dear...");
                ...
                if (f == -1)
                {
                    ...
                    if (File.Exists(path))
                    {
                        ...
                        if (fi.Length != e.size)
                        {
                            ...
                            //Console.WriteLine((count++) + " : Entry will be updated " + e.name);
                            TOC.Entries[i] = e;
                        }
                    }
                    //else
                    //    Console.WriteLine((count++) + " : Entry not found " + e.name);
                }  /////////////////////////////// END KFREON BLATHER
                else
                {
                    //if (Files[f].UncompressedSize == e.size)
                    //    Console.WriteLine((count++) + " : Entry is correct " + e.name);
                    //else 
                    if (Files[f].UncompressedSize != e.size)
                    {
            ...
            //Console.WriteLine("Replacing TOC back...");
            ReplaceEntry(TOC.Save().ToArray(), IndexTOC);
            if (Rebuild)
            {
                //Console.WriteLine("Reopening SFAR...");
                Load(FileName);
