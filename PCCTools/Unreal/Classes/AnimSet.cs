...
namespace PCCTools.Unreal.Classes
{
    public class AnimSet
    {
        ...
        public bool ImportFromPSA(string path)
        {
            ...
            {
                Console.WriteLine("Cant import: different count of bones");
                return false;
            }
            for (int i = 0; i < SetData.TrackBoneNames.Count; i++)
                if (d.Bones[i].name != SetData.TrackBoneNames[i])
                {
                    Console.WriteLine("Cant import: couldnt match all bones");
                    return false;
                }
            if (d.Infos.Count != Sequences.Count)
            {
                Console.WriteLine("Cant import: different count of sequences");
                return false;
            }
            for (int i = 0; i < Sequences.Count; i++)
            {
                ...
                {
                    Console.WriteLine("Cant import: couldnt match all sequences");
                    return false;
                }
            }
            int pos = 0;
            for (int i = 0; i < Sequences.Count; i++)
            {
                Console.WriteLine("Importing into AnimSequence #" + Sequences[i] + " ...");
                AnimSequence ans = new AnimSequence(pcc, Sequences[i]);
